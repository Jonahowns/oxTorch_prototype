...
class Integrator(torch.nn.Module):
    def __init__(self, type, timestep, temperature, masses, device="cpu", otherparams=None):
        ...
        self.device = device
    def first_step(self, coords, vels, accs_last):
        if self.type == "vel":
            coords = coords + vels * self.timestep + 0.5 * accs_last * self.timestep * self.timestep
        elif self.type == "langevin":
            # From Gronbech-Jensen 2013
            # alpha, twokbT = thermostat_const, temperature
            alpha, twokbT = self.otherparams
            ...
            coords_last = coords
            coords = coords + b * self.timestep * vels + 0.5 * b * (self.timestep ** 2) * accs_last + 0.5 * b * self.timestep * beta / self.masses.unsqueeze(2)
        elif self.type == "langevin_simple":
            coords = coords + vels * self.timestep + 0.5 * accs_last * self.timestep * self.timestep

        return coords

    def second_step(self, coords, coords_last, vels, accs_last, accs):
        if self.type == "vel":
            vels = vels + 0.5 * (accs_last + accs) * self.timestep
            accs_last = accs
        elif self.type == "no_vel":
            coords_next = 2 * coords - coords_last + accs * self.timestep * self.timestep
            coords_last = coords
            coords = coords_next
        elif self.type == "langevin":
            # From Gronbech-Jensen 2013
            vels = vels + 0.5 * self.timestep * (accs_last + accs) - alpha * (coords - coords_last) / self.masses.unsqueeze(2) + beta / self.masses.unsqueeze(2)
            accs_last = accs
        elif self.type == "langevin_simple":
            gamma, twokbT = thermostat_const, temperature
            accs = accs + (-gamma * vels + np.sqrt(gamma * twokbT) * torch.randn(vels.shape, device=self.device)) / self.masses.unsqueeze(2)
            vels = vels + 0.5 * (accs_last + accs) * self.timestep
            accs_last = accs
        elif self.type == "min":
    ...
    def thermostat(self):
        # Apply thermostat
        if self.type in ("vel", "no_vel") and thermostat_const > 0.0:
            thermostat_prob = self.self.timestep / thermostat_const
...
class Simulator(torch.nn.Module):
    ...
    """
    def __init__(self, parameterdict, applicationdict):
    ...
    def sim_step(self, coords, masses, n_steps, integrator="vel", device="cpu", start_temperature=0.1, timestep=0.02,
        ...
        for i in range(n_steps):
            ...
            coords = grator.first_step(coords)
